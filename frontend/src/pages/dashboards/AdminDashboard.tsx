...
const AdminDashboard: React.FC = () => {
        ...
        try {
            if (activeTab === 'employers') {
                ...
                if (Array.isArray(data)) {
                    ...
                    console.error('Expected employers array, got:', data);
                    setEmployers([]);
                }
            } else {
                ...
                if (Array.isArray(data)) {
                    ...
                    console.error('Expected internships array, got:', data);
                    setInternships([]);
                }
            }
        } catch (err: any) {
            console.error('Admin Dashboard Fetch Error:', err);
            setError(err.detailedMessage || err.message || 'Failed to fetch data. Please check if you have admin permissions.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredEmployers = (employers || []).filter(emp => {
        ...
        const companyName = emp?.company_name?.toLowerCase() ?? '';
        const industry = emp?.industry?.toLowerCase() ?? '';
        
        return companyName.includes(searchTermLower) || industry.includes(searchTermLower);
    });

    const filteredInternships = (internships || []).filter(int => {
        const searchTermLower = searchTerm.toLowerCase();
        const title = int?.title?.toLowerCase() ?? '';
        const companyName = int?.employers?.company_name?.toLowerCase() ?? '';
        
        return title.includes(searchTermLower) || companyName.includes(searchTermLower);
    });
